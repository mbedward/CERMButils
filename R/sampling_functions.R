...
#'
#' @param sampline Sampling lines as generated by function
#'   \code{make_sample_lines}. Since that function returns a named list of two
#'   spatial data frames, this argument can either be the \code{'sample_lines'}
#'   element of that list or, for convenience, the list itself (see examples
#'   below).
#'
...
#' slines <- make_sample_lines(dat_bb, bb_id = "OBJECTID", trim_by = "all")
#'
#' # The slines object is a named list containing two spatial data frames, one
#' # (element 'sample_lines') contains the sample lines and the other (element
#' # 'bb_segments') contains the segments of the input back-burning lines
#' # closest to each of the sample lines.
#' #
#' # Now load a corresponding layer of fire progression polygons and get
#' # the date-time value(s), and possibly gaps of no data, along each sample
#' # line.
#' #
...
#'                                           slines$sample_lines)
#'
#' # For convenience, the above function can also be called with the list object
#' # returned by make_sample_lines()
#' #
#' dat_prog_samples <- get_progression_times(dat_prog,
#'                                           datetimecol = "LASTTIME",
#'                                           slines)
...
get_progression_times <- function(prog,
  ...
  # The sampline argument should either be an sf data frame of line features or
  # a list with an element named 'sample_lines' of line features (as returned by
  # the make_sample_lines function)
  if (inherits(sampline, "list")) {
    nms <- names(sampline)
    ok <- "sample_lines" %in% nms

    if (ok) {
      sampline <- sampline$sample_lines
    } else {
      stop("If argument sampline is a list it must contain an element named 'sample_lines'")
    }
  }
  ...
  res <- lapply(seq_len(nrow(sampline)), function(iline) {
    ...
    ii <- unique( unlist( st_intersects(ln, prog) ) )
    ...
    # Order the segments for each sample by their distance from the start of the
    # the sample line.
...
#'
#' @return A named list containing two \code{sf} spatial data frames:
#'   \code{'sample_lines'} and \code{'bb_segments'}. The \code{'sample_lines'}
#'   data frame has the following columns:
#'   \itemize{
#'   \item{\code{refid} Identifier of the back-burning line feature, as taken
#'   from the input data column specified by the \code{bb_id} argument.}
#'
#'   \item{\code{line_index} Integer index (from 1) of the sampling line within
#'   the given back-burning line.}
#'
#'   \item{\code{segment} Either \code{'L'} or \code{'R'} indicating the position
#'   of the sampling line on the left or right of the back-burning line. Note
#'   that left and right, as used here, are simply relative to the digitizing
#'   direction (i.e. order of vertices) of the back-burning.}
#'
#'   \item{\code{geom} The sampling line geometry (\code{LINESTRING}).}
#'   }
#'
#'   The \code{'bb_segments'} data frame contains line features, each of which
#'   is the segment of a back-burning line that is closest to a given sampling
#'   line. The data frame has the following columns:
#'   \itemize{
#'   \item{\code{refid} Identifier of the back-burning line feature, as taken
#'   from the input data column specified by the \code{bb_id} argument.}
#'
#'   \item{\code{line_index} Integer index (from 1) of the sampling lines within a
#'   given back-burning line.}
#'
#'   \item{\code{geom} The line segment geometry (\code{LINESTRING}).}
#'   }
#'
#'
...
#' #
#' res <- make_sample_lines(bb,
#'                          line_spacing = 1000,
#'                          line_half_length = 5000,
#'                          trim_by = "all")
#'
#' # res is a list containing two sf data frames
#' head(res$sample_lines)
#' head(res$bb_segments)
#' }
#'
#' @export
#
make_sample_lines <- function(bb_lines,
  ...
  res <- lapply(seq_len(nrow(bb_lines)), function(line_index) {
    ...
    # Get the vertex coordinates of the original and smoothed line features
    vs <- sf::st_coordinates(g)[, 1:2]
    gsmooth_vertices <- sf::st_coordinates(gsmooth)[, 1:2]

    # Calculate the distance of each vertex along the original (unsmoothed) line feature
    d <- sapply( seq_len(nrow(vs)-1), function(k) sqrt(sum((vs[k,] - vs[k+1,])^2)) )
    d <- c(0, cumsum(d))

    # Locate the vertex closest to the middle of the feature where the first
    # sample line will cross
    dmax <- max(d)
    imid <- which.min( abs(d - dmax/2) )
    step_points <- icur <- imid

    # Now locate the vertices where other sample lines will cross
    #
    ...
    icur <- imid
    repeat {
      # Target distance of sample line along back-burning line
      dtarget <- d[icur] + line_spacing
    ...
    # Find the break-points that will define back-burning line segment(s)
    # associated with each sample line crossing. Here, 'associated' means the
    # portion of the back-burning line that is closest, in terms of along the
    # line distance, to each sample line origin.
    #
    segment_points <- 1

    if (length(step_points) > 1) {
      # Line is long enough to have multiple segments
      for (k in 2:length(step_points)) {
        dtarget <- (d[step_points[k]] + d[step_points[k-1]]) / 2
        icur <- which.min(abs(d - dtarget))
        segment_points <- c(segment_points, icur)
      }
    }

    segment_points <- c(segment_points, nrow(vs))

    # Assemble the back-burning line segments as an sf data frame
    line_segments <- lapply(2:length(segment_points), function(k) {
      v_indices <- seq(segment_points[k-1], segment_points[k])
      seg <- sf::st_linestring(vs[v_indices, ])
      seg <- sf::st_sfc(seg, crs = out_epsg)
      sf::st_sf(refid = FeatureID, line_index = k-1, geom = seg)
    })
    line_segments <- do.call(rbind, line_segments)

    # Create the pair of sample line segments at each step vertex, either side
    # of the back-burning line
    sample_lines <- lapply(seq_along(step_points), function(istep) {
    ...
    list(sample_lines = sample_lines, line_segments = line_segments)
  })


  # Aggregate the sets of sample lines and back-burning line segments into two sf data frames
  dat_sample_lines <- Map(\(x) x$sample_lines, res) %>% do.call(rbind, .)
  dat_line_segments <- Map(\(x) x$line_segments, res) %>% do.call(rbind, .)
  ...
  dat_sample_lines$segment_id <- NULL

  # Return results
  list(sample_lines = dat_sample_lines, bb_segments = dat_line_segments)
}
